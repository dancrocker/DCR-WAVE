...
# This is the orange line for intake depth
GTA_intake <- df[df$Taxa == "Grand Total" & year(df$Date) %in% vyear & df$Depth_m >= qintake_min & df$Depth_m <= qintake_max &
    df$Station %in% c(202),] %>%
  filter(Result != 8888, Result != 9999) %>% 
  group_by(Date) %>%
  drop_na(-PA) %>%
  summarize(Result = max(Result)) %>%
  mutate("intake")
names(GTA_intake) <- c("date","value","dataset")
GTA_intake$dataset <- factor(GTA_intake$dataset)
...
intake_leg <- paste0("Intake (", qintake_min, "-", qintake_max, "m)")
colors <- c("#0099FF", "black", "orange")
...
p  <- ggplot() +
  ...
  if ("202" %in% unique(df$Station)){
    p +
      geom_point(data = GTA_intake, aes(x = date, y = value, color = intake_leg), size = 3, shape = 17)
  }
  p <- p +
